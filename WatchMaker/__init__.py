...
class Prepare(object):
    ...
    def __init__(self, noreboot=False, s3=False, config_path=None, stream=False, log_path=None):
        ...
        :param stream: Enables logging to a file.
        :type stream: bool
        :param log_path: Path to logfile for stream logging.
    ...
    def _get_config_data(self):
        ...
        if not os.path.exists(self.config_path):
            logging.warning('{0} does not exist.  using the default config.'.format(self.config_path))
    ...
    def _get_system_params(self):
        ...
        if 'Linux' in self.system:
            ...
            logging.fatal('System, {0}, is not recognized?'.format(self.system))
        ...
        try:
            ...
            logging.fatal('Could not create a directory in {0}.\n'
    ...
    def _get_scripts_to_execute(self):
        ...
        for item in self.config[self.system]:
            try:
                ...
                logging.fatal('For {0} in {1} the parameters could not be merged'.format(
                    item,
                    self.config_path
                ))
                exceptionhandler(exc)
